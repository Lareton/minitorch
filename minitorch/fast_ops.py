    ...
    * When `out` and `in` are stride-aligned, avoid indexing
    ...
    def _map(
        ...
        if aligned:
            ...
            for i in prange(len(out)):
                # Each parallel iteration owns its index buffers.
    ...
    def _reduce(
        ...
        for i in prange(len(out)):
            ...
            for j in range(a_shape[reduce_dim]):
                value = fn(value, a_storage[a_pos + j * stride])
            out[out_pos] = value
...
def _tensor_matrix_multiply(
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    rows, cols = out_shape[1], out_shape[2]
    for index in prange(out_shape[0] * rows * cols):
        ...
        for k in range(a_shape[2]):
            value += a_storage[a_pos] * b_storage[b_pos]
            a_pos += a_strides[2]
            b_pos += b_strides[1]
